...
local cmd_extend = function(args)
  return vim.list_extend(
    vim.deepcopy(multiplexer_mux_wezterm.meta.cmd, true),
    args
  )
end
...
multiplexer_mux_wezterm.is_zoomed = function(opt)
  ...
  return utils.exec(command, function(p)
    ...
    for _, pane in pairs(data) do
      if
        pane.is_active == true
        and tostring(pane.pane_id) == multiplexer_mux_wezterm.meta.pane_id
      then
...
multiplexer_mux_wezterm.is_active = function(opt)
  ...
  return utils.exec(command, function(p)
    ...
    local requested_pane_id = opt and opt.id
      or multiplexer_mux_wezterm.meta.pane_id
    local data = vim.json.decode(p.stdout)
    if not data or next(data) == nil then
      vim.notify(
        'Failed to get clients info\n' .. p.stderr,
